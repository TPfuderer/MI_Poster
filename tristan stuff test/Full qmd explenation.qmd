...
Goal: simulate data, induce missingness, run multiple imputation methods, pool a linear model, and evaluate bias/coverage/variance.
...
* $R$ replications approximate repeated sampling.
* $m$ imputations controls Monte Carlo error of MI pooling.

---

## 2. Data-generating process (DGP)

We generate predictors $X \in \mathbb{R}^{n \times p}$ from a multivariate normal:

$$
X_i \sim \mathcal{N}_p(0, \Sigma)
$$

Then generate outcome:

$$
Y = \beta_0 + \beta_1 X_1 + \beta_2 X_2 + \beta_3 X_3 + \beta_4 X_8 + \beta_5 X_9

* \beta_6 X_3^2 + \beta_7 (X_1X_2) + \beta_8 (X_8X_9) + \varepsilon,
...
**Logic behind “derived terms”:**

-   We compute `X3_sq`, `X1_X2`, `X8_X9` to match the true model terms.

------------------------------------------------------------------------

## 3. Methods and storage

We compare:

-   `rf_ranger` (miceRanger)
-   `pmm` (mice)
-   `cart` (mice)
...
# store first 10 datasets for plotting/diagnostics later
...
-   We store pooled estimates per replication so we can compute bias/coverage across $R$.
...
-   Backup every 10 sims prevents losing all progress.

------------------------------------------------------------------------

## 5. Simulation loop

### 5.1 Generate full data + derived terms
...
### 5.2 Induce missingness (MAR / MCAR)

**MAR idea:** probability of missingness depends on *observed* drivers.

$$
\Pr(R=1 \mid X) = \text{logit}^{-1}(\eta),\quad \eta = -0.85 + 0.5 X_9 - 0.5 X_{10}
$$
...
-   MAR uses observed `X9, X10` as missingness drivers.
-   Derived terms are recomputed to respect NA structure.

------------------------------------------------------------------------

### 5.3 Imputation (3 methods)
...
-   Each method gets the *same* incomplete dataset `miss_df`.

------------------------------------------------------------------------

### 5.4 Diagnostics (RMSE on missing entries + correlation error)
...
-   RMSE only checks predictive recovery (not “properness”).
-   Correlation error checks whether covariance structure is preserved.

------------------------------------------------------------------------

### 5.5 Analysis model + Rubin pooling

We fit the same linear model to each imputed dataset and pool:

Rubin’s rules (scalar intuition):

$$
\bar{Q}=\frac1m\sum_{j=1}^m Q_j,\quad
\bar{U}=\frac1m\sum_{j=1}^m U_j,\quad
...
-   Pooling gives valid inference *if* MI is proper (goal of your project).

------------------------------------------------------------------------

### 5.6 Timing per method + backup + simulation timing
...
## 7. Evaluation per method (bias, coverage, variance, performance)

Coverage uses pooled $t$-interval:

$$
\hat\beta_k \pm t_{0.975,\ \text{df}} \cdot \widehat{\text{SE}}(\hat\beta_k)
$$
...
## 8. Save results with correct metadata
...
## Notes / tweaks you may want later

-   **Derived variables**: if you want strict coherence, do *not* impute `X3_sq`, `X1_X2`, `X8_X9` directly. Instead recompute after imputation.
-   **RMSE** currently uses only the 1st imputation; average across all `m` for stability.
