...
#############
#Load
burgette_results <- readRDS("W:/25_iterations_300sims_uff.rds")
...
coverage_table_summary <- plot_df %>%
  filter(Parameter != "(Intercept)") %>%       # optional, Intercept weglassen
  group_by(Method) %>%
  summarise(
    Mean_Coverage = mean(Coverage, na.rm = TRUE)
  ) %>%
  mutate(Mean_Coverage = round(Mean_Coverage, 3))

print(coverage_table_summary)
...
method_summary <- plot_df %>%
  ...
  group_by(Method) %>%
  summarise(
    Rel_Bias = mean(abs(Bias), na.rm=TRUE),
    Mean_Coverage = mean(Coverage, na.rm=TRUE)
  )
